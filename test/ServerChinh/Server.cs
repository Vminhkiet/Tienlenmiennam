...
namespace ServerChinh
{
    public partial class Server : Form
    {
        ...
        public delegate void XoaPhong(int port);
        public XoaPhong xoaPhong;
                             ...
                             for (int i = 0; i < 4; i++)
                             {
                                 ...
                                 if (players[i].taitk != "")
                                 {
                                     if (ktsttnaycodangchoikhong(i + 1))
                                         str += "-" + players[i].hoten + "," + players[i].Tien + "," + (i + 1).ToString() + ",Co";
                                     else
                                         str += "-" + players[i].hoten + "," + players[i].Tien + "," + (i + 1).ToString() + ",Ko";
                                 }
                             }
                         }
        ...
        void Recieve(object obj)
        {
            ...
            try
            {
                while (true)
                {
                    ...
                    else if (strings[0] == "Msg")
                    {
                        foreach(var i in clients)
                        {
                            i.Item1.Send(serialize("Msg-"+strings[1] + "-" + strings[2]));
                        }
                    }
                    else if (strings[0] == "Capnhat")
                    {
                        Thread thread = new Thread(() =>
                        {
                            ...
                            if (strings[1] == "Tatca")
                            {
                                ...
                                modify.TaiKhoans(hh);
                                str = "Capnhat-Tatca-" + strings[2] + "-" + s[0] + "," + p1.Tien + "-" + strings[4];
                                if (clients.Count() > 1)
                                {
                                    foreach (var i in clients)
                                    {
        ...
        void PhatBai()
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                {
                    clients[laychiso(i + 1)].Item1.Send(serialize(strs[i] + "-" + turn));
                }
            }
        ...
        string tinhtien(int win)
        {
            ...
            query = "Update TaiKhoan  SET Tien='" + players[win - 1].tien + "' where TenTaiKhoan = '" + players[win - 1].taitk + "'";
            modify.Command(query);
        ...
        private void Server_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            foreach (var i in clients)
            {
                query = "UPDATE TaiKhoan SET DN = 0 WHERE TenTaiKhoan = '" + players[i.Item2 - 1].taitk + "'";
                modify.Command(query);
            }
        }
