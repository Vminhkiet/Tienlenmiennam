...
namespace ServerChinh
{
    ...
    public int songuoi()
        ...
        void Taoban(string tentk,string mk,string tien,int client)
        {
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    form.xoaPhong = new Server.XoaPhong(XoaPhong);
                    a.Add(form);
        ...
        void choingayserver(string tentk, int banvuavao, int client)
        {
            ...
            int count = 0;
            string query1 = "Select * from TaiKhoan where TenTaiKhoan = '" + tentk + "'";
            decimal tienngchs = modify.TaiKhoans(query1)[0].Tien;
            while (a.Count() > 0)
            {
                count++;
                rand = random.Next(0, a.Count);
                port = a[rand].getport();
                tienb = xulytien(a[rand].tienphong);
                if (port != banvuavao && a[rand].songuoi() < 4 && !a[rand].kttkthoat(tentk) && a[rand].dangchoi == 0 && a[rand].getmk() == "" && tienb*13<tienngchs)
                {
                   
                    break;
                }
                if (port != banvuavao && a[rand].songuoi() < 4 && !a[rand].kttkthoat(tentk) && a[rand].getmk() == "" && tienb * 13 < tienngchs)
                {
                    dangchoi = true;
                    break;
                }
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            /* List<int> indicesToRemove = new List<int>();
             ...
             foreach (var server in a)
             {
                 if (server.dong)
                 {
                     lock (lockObject)
                     {
                         int index = a.IndexOf(server);
             ...
             // Xóa các phần tử từ b sử dụng các chỉ số
             lock (lockObject)
             {
                 foreach (int index in indicesToRemove)
                 {
                     Thread threadToRemove = b[index];
                     b.RemoveAt(index);
                     a.RemoveAt(index);
                     // Ngắt hoặc dừng luồng khi cần thiết
                     // threadToRemove.Interrupt();
                     // hoặc threadToRemove.Abort(); // Lưu ý: Sử dụng Thread.Abort không được khuyến nghị
                 }
             }
             kryptonLabel1.Text=a.Count() + "-" + b.Count;*/
        }
        private void SERVERCHINH_FormClosing(object sender, FormClosingEventArgs e)
        {
            string query;
            query = "UPDATE TaiKhoan SET DN = 0";
            modify.Command(query);
        }
    }
}
